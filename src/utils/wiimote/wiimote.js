...
export default class WIIMote{
    ...
    initiateDevice(){
        ...
        const openPromise = this.device.opened ? Promise.resolve() : this.device.open();
        
        openPromise.then(() => {
            console.log('HID device opened. Sending status request...');
            // Enable buttons and continuous reporting
    ...
    setDataTracking(dataMode = DataReportMode.CORE_BUTTONS){
        // 0x04 in the first byte enables continuous reporting
        return this.sendReport( ReportMode.DATA_REPORTING, [0x04, dataMode]);
    }
    ...
    BTNDecoder(byte1, byte2){
        const oldStatus = JSON.stringify(this.buttonStatus);
        for (let i = 0; i < 8; i++) {
        ...
        if(this.BtnListener != null && oldStatus !== JSON.stringify(this.buttonStatus)){
    ...
    listener(event){
        var data = new Uint8Array(event.data.buffer, event.data.byteOffset, event.data.byteLength);
        
        if(event.reportId == InputReport.STATUS){
            console.log('Status report received:', data);
            this.setDataTracking(DataReportMode.CORE_BUTTONS)
        }

        // The first two bytes of almost all reports (except 0x21, 0x22) are buttons
        if (data.length >= 2) {
            this.BTNDecoder(data[0], data[1]);
        }

        // If report includes accelerometer
        if (data.length >= 5 && (event.reportId == 0x31 || event.reportId == 0x33 || event.reportId == 0x35 || event.reportId == 0x37)) {
            this.ACCDecoder([data[2], data[3], data[4]]);
        }

        // If report includes IR (0x33, 0x36, 0x37) - simplified check
        if (data.length >= 17 && (event.reportId == 0x33 || event.reportId == 0x36 || event.reportId == 0x37)) {
            this.IRDecoder(data.slice(5, 17));
        }
    }
}
